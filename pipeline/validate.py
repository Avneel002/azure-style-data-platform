...
def validate_data(df, source_type):
    print(f"Validating {source_type} data")
    ...
    try:
        ...
        if nulls.any():
            report.add_warning(f"Null values detected: {nulls[nulls > 0].to_dict()}")
            ...
            if source_type == "csv":
                ...
                report.add_check(
                    "Null Handling",
                    "PASSED",
                    f"Removed {before - len(df_validated)} rows"
                )
            else:
                ...
                report.add_check("Null Handling", "PASSED", "Filled defaults")
        else:
            report.add_check("Null Check", "PASSED")
        ...
        report.add_check(
            "Deduplication",
            "PASSED",
            f"Removed {before - len(df_validated)} duplicates"
        )
        ...
        if source_type == "csv":
            ...
            if before != len(df_validated):
                report.add_warning(f"Removed {before - len(df_validated)} rows due to type issues")
        ...
        if source_type == "csv":
            ...
            report.add_check("Business Rules", "PASSED")
